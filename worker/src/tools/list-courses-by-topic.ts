...
export function makeListCoursesByTopic(env: ToolEnv) {
	return tool({
		description:
			"List Frontend Masters courses whose title matches a topic, technology, or keyword (e.g. 'React', 'CSS', 'GraphQL', 'TypeScript'). Use this when the user asks which courses exist on a subject or wants to browse courses by technology. This is an exact title-keyword lookup over the course catalog. It matches the course title only — for questions about what a course teaches, or topics not named in any title, use ragSearch instead.",
		...
		execute: async ({ topic }) => {
			...
			const results = await listCoursesForTopic(topic, db);
